...
Обычные и фаззинг-тесты запускаются одной командой:
...
```powershell
python manage.py test videos.tests_fuzz
```

Фаззинг-тесты находятся в `videos/tests_fuzz.py`. Они генерируют случайные SRT-строки, языковые коды, временные значения и некорректные HTTP-запросы, чтобы проверить, что приложение не падает с ошибкой уровня 500 и не раскрывает чужие задания.
